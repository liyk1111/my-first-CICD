...
#define ADDR_STM32_FLASH_END 	0x08100000
...
/*写入地址必须8的倍数*/
uint8_t FlashWrite(uint32_t WriteAddr,uint8_t *pBuffer,uint32_t NumToWrite);		//从指定地址开始写入指定长度的数据
//void STMFLASH_Write(uint32_t WriteAddr,uint32_t *pBuffer,uint32_t NumToWrite);
void FlashRead(uint32_t ReadAddr,uint8_t *pBuffer,uint32_t NumToRead) ;   		//从指定地址开始读出指定长度的数据
//测试写入
